...
public class RaeuberinnenProblem {
	...
	private static void saveStats(Beute b, AufteilungsAlgorithmus a, double[] sol, double time) {
		try {
			...
			csvWriter.newLine();
			csvWriter.write(String.join(",","Iterations needed",String.valueOf(a.getNumbSolvedGurobi())));
			csvWriter.newLine();
	...
	private static void saveSolution(Beute b, AufteilungsAlgorithmus a, double[] sol) { 
		try {
			...
			csvWriter.write(String.join(",","","Summe",String.valueOf(summen[0]),String.valueOf(summen[1]),String.valueOf(Math.abs(summen[0]-summen[1]))));
