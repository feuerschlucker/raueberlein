...
public class KP4
{
	...
	private static class Item
	{
		...
		{
			return name + ": profit = " + profit + ", weight = " + weight;
		}
...
//	private static final double CAPACITY = 434;
	...
	private static final double MAX_GAP = 1E-4;
	...
	private static boolean setRootBound = true;
	...
	private static boolean[] startHeuristic()
	{
	...
	private static double globalUpperBound2()
	{
		int[] sortedIndicesOfItems = new int[N];
		boolean[] used = new boolean[N];
		Arrays.fill(used, false);
		...
		for (int i = 0; i < N; i++)
		{
			...
			{
				remainingCapacity -= ITEMS[sortedIndicesOfItems[i]].getWeight();
				result += ITEMS[sortedIndicesOfItems[i]].getProfit();
			}
			else
			{
				result += ITEMS[sortedIndicesOfItems[i]].getProfit() * remainingCapacity / ITEMS[sortedIndicesOfItems[i]].getWeight();
				return result;
			}
		}
		return result;
	...
	private static double globalUpperBound3()
	{
		for (final Item item : ITEMS)
		{
			if (Math.round(item.getWeight()) != item.getWeight())
			{
				return globalUpperBound;
			}
		}
		
		return Math.floor(globalUpperBound);
	}
...
//	private static boolean[] branchAndBound(boolean[] smallerEqual, Double[] rSSE, boolean[] greaterEqual[], Double[] rSGE)

	{
		...
		if (model.get(GRB.IntAttr.Status) == GRB.Status.OPTIMAL)
		{
			...
			{
				setRootBound = false;
				if (model.get(GRB.DoubleAttr.ObjVal) < globalUpperBound)
				{
					globalUpperBound = model.get(GRB.DoubleAttr.ObjVal);
				}
			}
...
//			}
			...
			if (ovResult > globalLowerBound)
			{
				...
				if (((globalUpperBound - globalLowerBound) / globalUpperBound) <= MAX_GAP)
				{
					optimumFound = true;
				}
			}
	...
	{
		boolean[] startHeuristicSolution = startHeuristic();
		globalLowerBound = ov(startHeuristicSolution);
		System.out.println("lower bound = " + globalLowerBound);
		globalUpperBound = globalUpperBound1();
		System.out.println("upper bound = " + globalUpperBound);
		double globalUpperBoundTemp = globalUpperBound2();
		if (globalUpperBoundTemp < globalUpperBound)
		{
			globalUpperBound = globalUpperBoundTemp;
			System.out.println("upper bound = " + globalUpperBound);
		}
		globalUpperBoundTemp = globalUpperBound3();
		if (globalUpperBoundTemp < globalUpperBound)
		{
			globalUpperBound = globalUpperBoundTemp;
			System.out.println("upper bound = " + globalUpperBound);
...
//		System.out.println("upper bound = " + globalUpperBound);
		System.out.println("gap = " + ((globalUpperBound - globalLowerBound) / globalUpperBound));
		...
		{
			solution = branchAndBound(new boolean[N], new boolean[N]);
		}
